...
| Lab ID | Lab Adı | Seviye | Gün | Süre | Profil | Açıklama |
|---|---|---|---|---|---|---|
| `LAB-LOG-01` | **Centralized Logging Architecture: Collector to Elasticsearch** | CORE | Gün 5 | 45 dk | `logging` | Elasticsearch single-node kurulumu, REST API (`_cat/indices`), index yaşam döngüsü. |
| `LAB-LOG-02` | **Structured JSON Logging in Microservices** | CORE | Gün 5 | 30 dk | `logging` | Unstructured text vs JSON logs (`timestamp`, `level`, `trace_id`, `message`, `http_status`). |
